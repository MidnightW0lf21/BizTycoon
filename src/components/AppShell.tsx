    ...
    <div className="grid min-h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr]">
      <div className="hidden border-r bg-muted/40 md:block sticky top-0 h-screen">
        <div className="flex h-full max-h-screen flex-col gap-0">
          <AppLogo />
          <nav className="grid items-start px-2 py-2 text-sm font-medium lg:px-4 overflow-y-auto">
            {navItems.map(item => (
              <NavLink
                key={item.label}
                {...item}
      ...
      <div className="flex flex-col h-screen overflow-hidden">
        <header className="flex h-14 shrink-0 items-center gap-4 border-b bg-background px-4 lg:h-[60px] lg:px-6 sticky top-0 z-10">
          <Sheet>
